...
function exportSVG(p) {
  ...
  if (showLines) {
    const d = pts.map((pt, i) =>
      `${i === 0 ? 'M' : 'L'}${(cx + pt.x * sc).toFixed(2)},${(cy - pt.y * sc).toFixed(2)}`
    ).join(' ');
    body += `<path d="${d}" fill="none" stroke="${fg}" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round"/>`;
  }

  const circles = pts.map(pt =>
    `<circle cx="${(cx + pt.x * sc).toFixed(2)}" cy="${(cy - pt.y * sc).toFixed(2)}" r="${(sw / 2).toFixed(2)}" fill="${fg}"/>`
  ).join('');
  body += circles;
...
function exportPNG(p) {
  ...
  if (showLines) {
    ctx.beginPath();
    pts.forEach((pt, i) => {
      const x = cx + pt.x * sc, y = cy - pt.y * sc;
...
new p5(function(p) {
  ...
  function s2n(sx, sy) {
    ...
    return [
      ...
      Math.max(0, Math.min(1, (sy - PAD) / inner)),
    ];
  }
  ...
  function drawNavGrid() {
    ...
    if (!selected.length) {
      ...
      drawHersheyText(p, 'SELECT LETTERS', h / 2, h / 2 + 5, 9, hc, 'center');
      return;
    }
  ...
  function drawViewport() {
    ...
    if (!pts.length) {
      const hc = [178, 176, 170];
      drawHersheyText(p, 'SELECT A LETTER', cx, cy + 5, 9, hc, 'center');
      return;
    }
    if (showLines) {
      p.stroke(30);
      ...
      p.beginShape();
      for (const pt of pts) p.vertex(cx + pt.x * sc, cy - pt.y * sc);
      p.endShape();
    }
    p.noStroke();
    p.fill(30);
    for (const pt of pts) {
      p.ellipse(cx + pt.x * sc, cy - pt.y * sc, dotSize, dotSize);
    }
  }
