...
// Initialized by SLIDERS in setup — match defaults below if you change them
let numPts, dotSize, numStrokes, strokeSpacing;

// ─── Slider configuration — edit ranges and defaults here ────────────────────
const SLIDERS = [
  {
    id: 'nPts',          lblId: 'lbl-pts',     valId: 'val-pts',
  ...
  {
    id: 'dotSize',       lblId: 'lbl-dot',     valId: 'val-dot',
    label: 'DOT SIZE',      min: 1,   max: 100,  step: 1,  value: 3,
    apply: v => dotSize = v,
  },
  {
    id: 'numStrokes',    lblId: 'lbl-strokes', valId: 'val-strokes',
    label: 'STROKES',       min: 1,   max: 20,  step: 1,  value: 1,
    apply: v => numStrokes = v,
  },
  {
    id: 'strokeSpacing', lblId: 'lbl-spacing', valId: 'val-spacing',
    label: 'STROKE GAP',    min: 1,   max: 50,  step: 1,  value: 3,
    apply: v => strokeSpacing = v / 100,
  },
];
...
// Samples any text (single letter or word) into n normalized points
...
function getInterpolated() {
  ...
  // IDW power variants
  const idwPow = { linear: 1, idw: 2, cubic: 3 }[interpMethod];
  if (idwPow !== undefined) return idwBlend(anchors, dx, dy, n, idwPow);

  // Extrapolation variants (extrap, extrap2, extrap4, extrap8, …)
  const extrapM = interpMethod.match(/^extrap(\d*)$/);
  if (extrapM) return extrapBlend(anchors, dx, dy, n, extrapM[1] ? +extrapM[1] : 1);
...
function starVerts(cx, cy, outerR, innerR, n) {
  const pts = [];
  for (let i = 0; i < n * 2; i++) {
...
// Normalized tangent at pts[i], with y flipped for screen space
...
// ─── HTML UI helpers ──────────────────────────────────────────────────────────
function setLabel(id, text, size = 10) {
  document.getElementById(id).innerHTML = hersheyTextSVG(text, size);
}
...
function buildInterpList() {
  ...
  for (const m of INTERP_METHODS) {
    ...
    div.addEventListener('click', () => {
      interpMethod = m.val;
      list.querySelectorAll('.interp-opt').forEach(d => d.classList.remove('active'));
...
function buildShapeList() {
  ...
  for (const s of SHAPES) {
    const div = document.createElement('div');
    div.className = 'shape-opt' + (dotShape === s.val ? ' active' : '');
    div.innerHTML = hersheyTextSVG(s.label, 8);
    div.addEventListener('click', () => {
      dotShape = s.val;
      list.querySelectorAll('.shape-opt').forEach(d => d.classList.remove('active'));
      div.classList.add('active');
      redrawSketch();
    });
    list.appendChild(div);
...
function exportSVG(p) {
  ...
  const size = p.height, cx = size/2, cy = size/2, sc = size * 0.72;
  const strokeGap = sc * strokeSpacing;
  const bg = '#f5f5f0', fg = '#1e1e1e';
  ...
  for (let k = 0; k < numStrokes; k++) {
    ...
    const fa = fillDots ? `fill="${fg}"` : `fill="none" stroke="${fg}" stroke-width="${outlineWeight}"`;
    const la = `stroke="${fg}" stroke-width="${outlineWeight}" stroke-linecap="round"`;
    body += pts.map((pt, i) => {
      ...
      switch (dotShape) {
        ...
        case 'star': {
          const sv = starVerts(x, y, r, r*0.4, 5).map(([vx,vy]) => `${vx.toFixed(2)},${vy.toFixed(2)}`).join(' ');
          return `<polygon points="${sv}" ${fa}/>`;
        }
      }
    }).join('');
  }
...
function exportPNG(p) {
  const pts = getInterpolated();
  if (!pts.length) return;
  const size = p.height * 2, cx = size/2, cy = size/2, sc = size * 0.72;
  const strokeGap = sc * strokeSpacing;
  ...
  for (let k = 0; k < numStrokes; k++) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      switch (dotShape) {
        ...
        case 'star': {
          const v = starVerts(x, y, r, r * 0.4, 5);
          ctx.moveTo(v[0][0], v[0][1]);
          for (let j = 1; j < v.length; j++) ctx.lineTo(v[j][0], v[j][1]);
          ctx.closePath();
          break;
        }
      }
...
new p5(function(p) {
  ...
  function drawNavGrid() {
    ...
    if (!selected.length) {
      ...
      drawHersheyText(p, 'SELECT 2 OR MORE', h/2, h/2+5, 9, hc, 'center');
      return;
    }
  ...
  function drawViewport() {
    const h = p.height, cx = h+h/2, cy = h/2, sc = h*0.72;
    const strokeGap = sc * strokeSpacing;
    const pts = getInterpolated();
    if (!pts.length) {
      const hc = [178,176,170];
      drawHersheyText(p, 'SELECT 2 OR MORE', cx, cy+5, 9, hc, 'center');
      return;
    }
    for (let k = 0; k < numStrokes; k++) {
      const s = sc + (k - (numStrokes-1)/2) * strokeGap;
      ...
      const col = 30;
      if (dotShape === 'line') {
        p.noFill(); p.stroke(col); p.strokeWeight(outlineWeight);
      } else if (fillDots) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        switch (dotShape) {
          ...
          case 'star': {
            const v = starVerts(sx, sy, r, r * 0.4, 5);
            p.beginShape();
            for (const [vx, vy] of v) p.vertex(vx, vy);
            p.endShape(p.CLOSE);
            break;
          }
        }
  ...
  p.setup = function() {
    ...
    for (const s of SLIDERS) {
      ...
      setLabel(s.lblId, s.label);
      setLabel(s.valId, String(s.value));
      s.apply(s.value);
      el.addEventListener('input', function() {
        setLabel(s.valId, this.value);
        s.apply(+this.value);
        p.redraw();
      });
    }
    ...
    document.getElementById('fillBtn').addEventListener('click', function() {
      fillDots = !fillDots;
      this.innerHTML = hersheyTextSVG(fillDots ? 'FILL ON' : 'FILL OFF', 10);
      p.redraw();
    });
