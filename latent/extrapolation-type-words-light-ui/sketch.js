...
const SLIDERS = [
  {
    id: 'nPts',          lblId: 'lbl-pts',     asciiId: 'ascii-pts',
  ...
  {
    id: 'dotSize',       lblId: 'lbl-dot',     asciiId: 'ascii-dot',
    label: 'DOT SIZE',      min: 1,   max: 100,  step: 1,  value: 3,
    apply: v => dotSize = v,
  },
  {
    id: 'numStrokes',    lblId: 'lbl-strokes', asciiId: 'ascii-strokes',
    label: 'STROKES',       min: 1,   max: 20,  step: 1,  value: 1,
    apply: v => numStrokes = v,
  },
  {
    id: 'strokeSpacing', lblId: 'lbl-spacing', asciiId: 'ascii-spacing',
    label: 'STROKE GAP',    min: 1,   max: 50,  step: 1,  value: 3,
    apply: v => strokeSpacing = v / 100,
  },
];

// Renders a slider's value as a dashed ASCII-style track, e.g. "----[ 25 ]--------",
// drawn with the same Hershey stroke font as the rest of the UI.
function renderAsciiTrack(s, value) {
  const TOTAL = 34;
  const bracket = `[ ${value} ]`;
  const avail = Math.max(4, TOTAL - bracket.length);
  const pct = (value - s.min) / (s.max - s.min);
  const left = Math.round(pct * avail);
  const right = avail - left;
  document.getElementById(s.asciiId + '-l').innerHTML = hersheyTextSVG('-'.repeat(left), 9);
  document.getElementById(s.asciiId + '-v').innerHTML = hersheyTextSVG(bracket, 9);
  document.getElementById(s.asciiId + '-r').innerHTML = hersheyTextSVG('-'.repeat(right), 9);
}
...
// Plain (real, non-vector) text labels — used for slider/section titles so they render in Diatype Mono.
...
new p5(function(p) {
  ...
  function drawViewport() {
    ...
    if (!pts.length) {
      const hc = [178,176,170];
      drawHersheyText(p, 'SELECT 2 OR MORE LETTERS', cx, cy+5, 9, hc, 'center');
      return;
    }
    for (let k = 0; k < numStrokes; k++) {
      const s = sc + (k - (numStrokes-1)/2) * strokeGap;
  ...
  p.setup = function() {
    ...
    for (const s of SLIDERS) {
      ...
      renderAsciiTrack(s, s.value);
      s.apply(s.value);
      el.addEventListener('input', function() {
        renderAsciiTrack(s, +this.value);
        s.apply(+this.value);
        p.redraw();
      });
    }
